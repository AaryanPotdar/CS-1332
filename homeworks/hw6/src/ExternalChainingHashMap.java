...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        int index = hash(key);
        for (ExternalChainingMapEntry<K, V> temp = table[index]; temp != null; temp = temp.getNext()) {
    ...
    /**
     * Helper hash method to calculate hash key.
     * @param key key value
     * @return int value
     */
    ...
    public V get(K key) {
        ...
        ExternalChainingMapEntry<K, V> curr = table[hash(key)];
        while (curr != null) {
    ...
    public Set<K> keySet() {
        ...
        for (ExternalChainingMapEntry<K, V> temp : table) {
            while (temp != null) {
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new IllegalArgumentException("length is less than size of HashMap");
        }
        ExternalChainingMapEntry<K, V>[] resizedTable = new ExternalChainingMapEntry[length];
        for (ExternalChainingMapEntry<K, V> temp : table) {
            while (temp != null) {
                int index = Math.abs(temp.getKey().hashCode() % length);
                ExternalChainingMapEntry<K, V> next = temp.getNext();
                temp.setNext(resizedTable[index]);
                resizedTable[index] = temp;
                temp = next;
            }
        }
        table = resizedTable;

 ...
 *
 * Resources: Junits tests shared on guthub
 */
public class MaxHeap<T extends Comparable<? super T>> {
    ...
    private void heapify(int i) {
        ...
        T lChild = backingArray[i * 2];
        T rChild = null;
        if ((i * 2) + 1 <= size && backingArray[(i * 2) + 1] != null) {
            rChild = backingArray[(i * 2) + 1];
        }
        T largestChild = lChild;
        int newIdx = i * 2;
        if (rChild != null) {
        ...
        if (backingArray[i].compareTo(largestChild) < 0) {
            backingArray[newIdx] = backingArray[i];
            backingArray[i] = largestChild;
            heapify(newIdx);
        }
    }

    /**
     * Helper method to swap parent and child.
     * @param i index i
     * @param j index j
     */
    ...
    public T getMax() {
        if (size == 0) {
            throw new NoSuchElementException("Cannot retrieve max from empty heap");
        }
